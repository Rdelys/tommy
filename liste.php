...
// Connexion à la base de données (remplace par tes informations de connexion)
...
// Si la catégorie sélectionnée est "Autres"
if (strtolower($category) == 'autres') {
    // Récupérer tous les produits dont la catégorie ne correspond pas à 'Immobilier', 'Véhicules', ou 'Meubles'
    $stmt = $conn->prepare("SELECT titre, description FROM produitdetail WHERE LOWER(categorie) NOT IN ('immobilier', 'voiture', 'meubles')");
    $stmt->execute();
    $result = $stmt->get_result();
    $products = $result->fetch_all(MYSQLI_ASSOC);
} else {
    // Si la catégorie est une des catégories valides, on effectue la requête
    $valid_categories = ['immobilier', 'voiture', 'meubles'];
    if (in_array(strtolower($category), $valid_categories)) {
        // Préparer la requête SQL pour la catégorie spécifiée
        $stmt = $conn->prepare("SELECT titre, description FROM produitdetail WHERE LOWER(categorie) = LOWER(?)");
        $stmt->bind_param('s', $category);
        $stmt->execute();
        $result = $stmt->get_result();
        $products = $result->fetch_all(MYSQLI_ASSOC);
    } else {
        // Si aucune catégorie n'est sélectionnée ou qu'il y a une erreur, on affiche une liste vide
        $products = [];
    }
}
...
<body>
    ...
    <main>
    <h3 style="text-align: center; color: #333; font-family: 'Arial', sans-serif; margin-bottom: 20px;">Produits dans la catégorie: <?= ucfirst($category) ?></h1>

    <?php if (count($products) > 0): ?>
        <ul style="list-style-type: none; padding: 0; margin: 0;">
            <?php foreach ($products as $product): ?>
                <li style="background-color: #f9f9f9; border-radius: 10px; padding: 15px; margin-bottom: 10px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);">
                    <h4 style="color: #333; font-size: 18px; font-family: 'Arial', sans-serif; margin-bottom: 10px;"><?= htmlspecialchars($product['titre']) ?></h3>
                    <p style="font-size: 14px; color: #666; line-height: 1.6;"><?= htmlspecialchars($product['description']) ?></p>
                </li>
            <?php endforeach; ?>
        </ul>
    <?php else: ?>
        <p style="text-align: center; font-size: 16px; color: #ff4d4d; font-family: 'Arial', sans-serif;">Aucun produit trouvé pour cette catégorie.</p>
    <?php endif; ?>
</main>
